...
and the surrounding text), which is caused by small errors in the Poisson solve. The rotation source term for
the momentum is coordinate-dependent; specifically, because the coordinates used in determining the Coriolis
force are cell-centered, there will not be (implied) exact cancellation at zone edges as there is for gravity.
