...
\begin{figure}[h]
  ...
  \caption[Angular momentum conservation properties for $f_R = 1.0$]
...
\begin{figure}[h]
  ...
  \caption[Angular momentum conservation properties for $f_R = 1.0$, grid losses subtracted]
...
and the surrounding text), which is caused by small errors in the Poisson solve. By contrast, even though the
centrifugal force is conservative when integrated over the whole domain, even if it is evaluated at zone centers,
the rotation source term for the momentum will behave differently if the velocities are evaluated at cell centers.
There will therefore not be (implied) exact cancellation at zone edges as there is for gravity.
...
by running the simulation a little longer), because the non-conservation effect here depends on the Coriolis
force, which is proportional to velocity, and which therefore becomes less important as the system reaches
...
representative of how the mergers would physically occur, but we include these for completeness.
...
\autoref{sec:mergers:stable}, for a $0.9\ \msolar$ primary and $0.6\ \msolar$ secondary, with the
change $f_R = 0.9$, so that the stellar centers of mass are 10\% closer to each other. This
...
equivalent Keplerian orbital period for this system is 52 seconds, so the timescale for
...
signal has the expected characteristics for such an event: initially, the signal has a
constant amplitude and an oscillation period equal to half the orbital frequency. As
significant mass transfer sets in, the frequency increases because of rapid coalescence,
and then the system eventually relaxes to an equilibrium (the ringdown phase). 
...
\label{sec:unstable:equal}

Equal-mass mergers are physically unlikely to occur, but are convenient for
simulation purposes because of their symmetry, and are maximally unstable against
mass transfer so they provide a limiting case for how violent the coalescence phase is.
We use two $0.9\ \msolar$ mass stars for the simulation shown here, again with $f_R = 0.9$
and an effective 200km resolution. The inertial frame evolution is displayed in
...
leaving a single merged remnant, within two orbital periods. The corresponding gravitational
wave signal is shown in \autoref{fig:unsteady_gw_signal_equal}.
